...
### 1.3 整体分层架构
#### 1）撮合逻辑
![流程图](docs/data/线程池架构优化为Disruptor架构.png)
![数据持久化服务流程](docs/data/05-DBService.png)
优点：
1. 提升TPS：Disruptor **无锁队列**（替代BlockingQueue和线程池）+ **股票分片**（Sharding）的串行撮合 + 内存 OrderBook 无锁高速撮合；
   - 灵活使用C++高效撮合模块；
2. 提升安全性：**幂等性**三层防护（布隆过滤器+ Redis+ DB）；
3. 性能&安全：异步、批量入库+ 指数退避**重试兜底**，不丢数据+ Druid**连接池**；
4. 功能：价格优先+ 时间次优先等基础功能；
5. 灵活：可拆卸自敲检测和合规验证模块，IPC通信；
6. 可观测：使用Prometheus+Grafana全流程可观测，订单成交情况、TPS、SQL、JVM全流程定制化监测（2.7）。

#### 2）崩溃恢复
1. 崩溃恢复的数据加载和主动撮合都在**分片服务**中进行；
2. CountDownLatch进行任务等待和唤醒；
3. 分片加载数据库数据进行处理。
#### 3）撤单
1. 扯淡全流程都在**分片服务**中；
2. 复用异步写数据库能力。
...
- 🚀 **撮合与持久化解耦架构**
  分片撮合+ Disruptor队列+ DB两重线程池（重试线程池隔离问题）+ Druid连接池+ 连接池Gate控制请求速度+ 对象池控制disruptor持久化任务创建回收+ 
   Redis+布隆过滤器（幂等验证）
